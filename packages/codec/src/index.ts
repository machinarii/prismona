// @prismona/codec — the open PRSM share-code format.
//
// A Prismona share code is a consent-carried bearer token for a personality
// profile: six domain z-scores (Big Five reported with Emotional Stability +
// HEXACO Honesty-Humility), an edition tier, a date, and a consistency
// index, packed into 12 bytes and base64url-encoded with a PRSM- prefix.
// Self-contained on purpose: no dependencies, decodes entirely offline.
// Parity with the prismona.vercel.app implementation is enforced by the
...
export function encodeShareCode(p: ShareProfile | { tier: Tier; date: string; traits: Record<ReportKey, { z: number }>; quality: { consistency: number } }): string {
  const z = "traits" in p
...
// Abramowitz & Stegun 26.2.17 normal CDF — percentile helper for consumers.
