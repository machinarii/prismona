...
// PUT saves this browser's profile bundle to the account; GET loads it on
// another device; DELETE removes it. Nothing syncs automatically.
...
export async function GET(req: Request): Promise<Response> {
  ...
  const data = await loadSyncedProfile(session.email);
...
export async function PUT(req: Request): Promise<Response> {
  ...
  await saveSyncedProfile(session.email, data);
...
export async function DELETE(req: Request): Promise<Response> {
  const session = sessionFrom(req);
  if (!session) return Response.json({ error: "not signed in" }, { status: 401 });
  await deleteSyncedProfile(session.email);
  return Response.json({ ok: true });
}
